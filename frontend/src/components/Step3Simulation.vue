    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? '시작 중...' : '결과 리포트 생성 시작' }}
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 이전 시뮬레이션 로그를 정리하고 새로 시작합니다')
      }
      ...
      addLog(`✗ 시작 실패: ${res.error || 'unknown error'}`)
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`중지 실패: ${res.error || 'unknown error'}`)
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('리포트 생성 요청 전송됨, 잠시 기다려주세요...')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ 리포트 생성 작업 시작: ${reportId}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ 리포트 생성 시작 실패: ${res.error || 'unknown error'}`)
